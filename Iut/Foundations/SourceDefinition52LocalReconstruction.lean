...
-/
import Iut.Foundations.SourceFinitePlaceMLFBase
...
noncomputable instance closureIsGalois
    (place : NumberField.FinitePlace K) :
    IsGalois (Base place) (Closure place) :=
  IsAlgClosure.isGalois (Base place) (Closure place)
...
noncomputable instance stageContinuousSMulRational
  ...
  exact IsScalarTower.algebraMap_apply
    (RationalBase place) (Base place) stage scalar
...
noncomputable abbrev module
    (place : NumberField.FinitePlace K) :
    SourceFiniteIndTopologicalLocalModule
      (ThetaFinitePlace.comap (k := ℚ) place) where
...
theorem continuous_stageToCarrier
  ...
  rw [show (stageToCarrier place stage : stage → (module place).carrier) =
      fun value => (module place).colimitCocone.ι.app stage value by
    funext value
