...
theorem finiteEdgeComponentEquiv_naturality
  ...
  | _ point =>
      ...
      have hnatural := finiteEdgeIdentification_inv_naturality
        diagram root map (coverReferenceBranch diagram root edge)
      have hhom := congrArg (fun arrow ↦ arrow.hom.hom) hnatural
      simp only [ObjectProperty.FullSubcategory.comp_hom, Action.comp_hom]
        at hhom
      have hpoint := ConcreteCategory.congr_hom hhom point
      simp only [EtaleFundamentalGroup.finiteTemperification_map_apply,
        SourceCountableTypeCat.comp_apply] at hpoint
      simp only [finiteCovMap, id_eq]
      exact hpoint.symm
