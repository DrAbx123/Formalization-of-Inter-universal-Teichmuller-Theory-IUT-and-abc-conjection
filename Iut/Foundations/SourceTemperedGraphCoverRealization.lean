...
theorem liftedEdge_coincidence_of_none
  ...
  have finiteCoincidence :
    ...
    generalize edgeEquality : liftedEdge.edge = edgeValue
      at branch noVertex ⊢
    rcases edgeValue with ⟨baseEdge, component⟩
    exact finiteEtaleCoverSemiGraph_coincidence_of_none
      diagram root object (component := component) noVertex
  ...
  · rintro ⟨⟨targetEdge, targetBranch⟩, targetCoincidence⟩
    ...
    have targetExists : ∃ targetVertex,
      ...
      simpa only [sourceBranch, Equiv.apply_symm_apply,
        SourceSemiGraph.coincidenceTotal] using targetCoincidence
    ...
    have mappedVertexEquality :
      ...
      exact mappedCoincidence.symm.trans <| by
        simpa only [sourceBranch, Equiv.apply_symm_apply,
          SourceSemiGraph.coincidenceTotal]
