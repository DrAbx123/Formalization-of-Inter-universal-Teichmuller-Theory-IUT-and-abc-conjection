...
theorem selectedEdgeComponent_refinement
  ...
  have componentNaturality :=
    (EtaleFundamentalGroup.fiberComponentFiniteActionEquiv_naturality
      (diagram.edgeAnabelioid lifted.edge.1)
      (reference.pullback.map (refinement.val.app reference.vertex))
      lifted.edge.2).symm
  change SourceSemiGraphOfAnabelioids.CovObject.actionComponentMap
      ((diagram.edgeAnabelioid lifted.edge.1).finiteTemperification.map
        (reference.pullback.map (refinement.val.app reference.vertex)))
      (SourceFiniteLevelUniversalCover.selectedEdgeComponent
        diagram root finer.object finer.rootVertex lifted.edge.1
          ⟨lifted, rfl⟩) =
    SourceFiniteLevelUniversalCover.selectedEdgeComponent
      diagram root coarser.object coarser.rootVertex lifted.edge.1
        (refinementEdgeIndexMap diagram root refinement lifted.edge.1
          ⟨lifted, rfl⟩) at componentNaturality
  exact componentNaturality
  ...
  change
    (diagram.edgeAnabelioid edge).fiber.map
      ...
      (SourceSemiGraphOfAnabelioids.CovObject.finiteBranchActionIso
          diagram root coarser.object branch).hom.hom.hom
        ((diagram.vertexAnabelioid branch.vertex).fiber.map
          (refinement.val.app branch.vertex) point) at pointEquality
  exact pointEquality
  ...
  · intro first second equality
    ...
    | _ first =>
      ...
      | _ second =>
        ...
        apply Prod.ext
        · calc
            ((correction * finerTransformation) • second).1 =
                (correction * finerTransformation) • second.1 := rfl
            _ = correction • (finerTransformation • second.1) :=
              mul_smul correction finerTransformation second.1
            _ = first.1 := correctionMaps
