...
theorem branchEdgeFamilyIso_inv_edgeBasePoint_val
  ...
  have restrictedValue' :
    ...
    simpa only [SourceFiniteLevelUniversalCover.branchEdgeFamilyIso,
      restrictedIso, familyIso, Iso.trans_inv,
      ObjectProperty.FullSubcategory.comp_hom, Action.comp_hom,
      SourceCountableTypeCat.comp_apply] using restrictedValue
...
theorem branchRepresentativeCorrection_smul_branchPreimageBase
  ...
  have representativeEquality := congrArg
    (fun index ↦
      (SourceFiniteLevelUniversalCover.selectedVertexComponent
        diagram root level.object level.rootVertex branch.vertex index).out)
    indexEquality
  change (SourceFiniteLevelUniversalCover.selectedVertexComponent
      diagram root level.object level.rootVertex branch.vertex pulled.1).out =
    (SourceFiniteLevelUniversalCover.selectedVertexComponent
      diagram root level.object level.rootVertex branch.vertex decoded.1).out
  ...
  simpa only [SourceFiniteLevelUniversalCover.selectedVertexComponent]
    using TargetSheetLift.branchRepresentativeCorrection_spec
      diagram root level.object liftedEdge.edge branch.branch
        branch.vertex branch.abuts
...
theorem liftedEdge_coincidence_at_branch
  ...
  dsimp only
  simpa only [SourceFiniteLevelUniversalCover.liftedEdgeBranchEquiv]
    using SourceFiniteLevelUniversalCover.liftedEdge_coincidence
      diagram root level.object level.rootVertex ⟨liftedEdge, rfl⟩
        branch.branch branch.vertex branch.abuts
...
theorem correctedTargetPoints_at_branch
  ...
  have compatibility := TargetSheetLift.branchCarrierEquiv_edgePoint
    diagram root target level.object level.rootVertex initial
      liftedEdge branch.branch decoded.1.1 coincidence
  simpa only [decoded] using compatibility
