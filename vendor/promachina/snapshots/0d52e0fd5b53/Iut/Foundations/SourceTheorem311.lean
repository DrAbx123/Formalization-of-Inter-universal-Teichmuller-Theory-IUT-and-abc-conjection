...
/-- The componentwise isomorphisms at one absolute-label procession position. -/
private noncomputable def IsomorphismMember.processionComponentMember
    {source target : SourceDAbsoluteThetaBridgeCore models}
    (iso : IsomorphismMember source target) :
    ∀ position,
      (CategoryCapsule.FullPolyMorphism.mk
        (source := source.procession.capsule position)
        (target := target.procession.capsule position)
        (Function.Embedding.refl
          (source.procession.capsule position).index)).Members := by
...
noncomputable def IsomorphismMember.processionHom
  ...
  component position :=
    { indexEmbedding := Function.Embedding.refl _
      members_nonempty :=
        ⟨iso.processionComponentMember position⟩ }
...
noncomputable def IsomorphismMember.processionHomMember
    {source target : SourceDAbsoluteThetaBridgeCore models}
    (iso : IsomorphismMember source target) :
    ∀ position, (iso.processionHom.component position).Members :=
...
/--
A horizontal evaluation invariant under the two equivalence indeterminacies
of Theorem 3.11(i).

`(Ind3)` is deliberately absent: Theorem 3.11(ii) makes it a one-way
upper-semi relation, not an equality generator.
-/
structure SourceTheorem311HorizontalEvaluationCompatibility
    {G1 : Type u} [Group G1] {G2 : Type w} [Group G2]
    {X : Type v} (Y : Type u)
    (ind1 : SourceEquivalenceAction G1 X)
    (ind2 : SourceEquivalenceAction G2 X) where
  evaluation : X -> Y
  ind1_compatible :
...
/-- The compatible horizontal evaluation descends to the exact Ind1/Ind2 quotient. -/
def descend
    (compatibility :
      SourceTheorem311HorizontalEvaluationCompatibility
        Y ind1 ind2) :
    SourceTheorem311IndeterminacyQuotient ind1 ind2 -> Y :=
...
theorem descend_mk
    ...
    compatibility.descend
        (Quotient.mk
          (sourceTheorem311IndeterminacySetoid ind1 ind2) value) =
...
/--
A source-facing `(Ind3)` evaluation law.

The target relation records the orientation of the conclusion: it may be set
inclusion, `Set.MapsTo`, relational surjectivity, or `(· ≤ ·)` for the
log-volume comparison.  No reverse implication is added.
-/
structure SourceTheorem311Ind3DirectionalCompatibility
    {X : Type v} {Y : Type u}
    (ind3 : X -> X -> Prop)
    (targetRelation : Y -> Y -> Prop) where
  evaluation : X -> Y
  map_step :
    ∀ {first second}, ind3 first second ->
      targetRelation (evaluation first) (evaluation second)

namespace SourceTheorem311Ind3DirectionalCompatibility

variable {X : Type v} {Y : Type u}
variable {ind3 : X -> X -> Prop} {targetRelation : Y -> Y -> Prop}

/-- A one-way `(Ind3)` step maps to the target relation in the same direction. -/
theorem map
    (compatibility :
      SourceTheorem311Ind3DirectionalCompatibility ind3 targetRelation)
    {first second : X}
    (related : ind3 first second) :
    targetRelation
      (compatibility.evaluation first)
      (compatibility.evaluation second) :=
  compatibility.map_step related

end SourceTheorem311Ind3DirectionalCompatibility
...
/-- Identity action used to audit the exact source-facing quotient boundary. -/
def sourceTheorem311IdentityAction
    (G : Type u) [Group G] (X : Type v) :
    SourceEquivalenceAction G X where
  act _ := Equiv.refl X
  one_act _ := rfl
  mul_act _ _ _ := rfl

/-- The strict one-way relation `false → true` used by the Ind3 regression. -/
def sourceTheorem311StrictBoolInd3 (first second : Bool) : Prop :=
  first = false ∧ second = true

theorem sourceTheorem311StrictBoolInd3_forward :
    sourceTheorem311StrictBoolInd3 false true :=
  ⟨rfl, rfl⟩

theorem sourceTheorem311StrictBoolInd3_not_reverse :
    ¬ sourceTheorem311StrictBoolInd3 true false := by
  simp [sourceTheorem311StrictBoolInd3]

private theorem sourceTheorem311IdentityRelation_eq
    {G1 : Type u} [Group G1] {G2 : Type w} [Group G2]
    {first second : Bool}
    (related :
      sourceTheorem311IndeterminacySetoid
        (sourceTheorem311IdentityAction G1 Bool)
        (sourceTheorem311IdentityAction G2 Bool)
        first second) :
    first = second := by
  induction related with
  | rel first second step =>
...
/--
Regression: the strict one-way Ind3 step is not an equality in the exact
source-facing quotient, which contains only the identity Ind1/Ind2 actions.
-/
theorem sourceTheorem311StrictBoolInd3_not_quotient_eq :
    let ind1 :=
      sourceTheorem311IdentityAction (Equiv.Perm (Fin 1)) Bool
    let ind2 :=
      sourceTheorem311IdentityAction (Equiv.Perm (Fin 1)) Bool
    (Quotient.mk (sourceTheorem311IndeterminacySetoid ind1 ind2) false :
        SourceTheorem311IndeterminacyQuotient ind1 ind2) ≠
      Quotient.mk (sourceTheorem311IndeterminacySetoid ind1 ind2) true := by
  dsimp
  intro hquotient
  have hfalse_true : false = true :=
    sourceTheorem311IdentityRelation_eq (Quotient.exact hquotient)
  simp at hfalse_true
