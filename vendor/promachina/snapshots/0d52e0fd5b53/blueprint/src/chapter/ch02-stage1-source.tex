...
\begin{definition}[Initial theta and theater data]
  ...
  \lean{Iut.ThetaFQParameterOrderData.OrdersPrimeToL}
  \leanok
  ...
  \(\mathrm{SL}_2(\mathbb Z/l\mathbb Z)\), and positive q-parameter orders
  prime to \(l\) at the bad \(F\)-places are explicit predicates and data
  rather than free propositions.
\end{definition}
...
\begin{definition}[Source Frobenioid prime strips]
  ...
  \lean{Iut.SourceDPrimeStrip.canonical}
  \lean{Iut.SourceDMonoAnalyticPrimeStrip.canonical}
  \lean{Iut.SourceFPrimeStrip}
  \lean{Iut.SourceFPrimeStrip.canonical}
  \lean{Iut.SourceFPrimeStrip.associatedDNonarchimedeanEquivalence}
  \lean{Iut.SourceFPrimeStrip.associatedD_archimedean_eq}
  ...
  \lean{Iut.SourceSelectedFinitePlace.integralIndSystem}
  \lean{Iut.SourceSelectedFinitePlace.integralIndSystemLimit}
  \lean{Iut.SourceSelectedFinitePlace.integralIndSequentialPresentation}
  \lean{Iut.SourceFMonoAnalyticPrimeStrip}
  \lean{Iut.SourceFMonoAnalyticPrimeStrip.canonical}
  \lean{Iut.sourceFMonoAnalyticPrimeStripCategory}
  \lean{Iut.SourceFMonoAnalyticPrimeStripCapsule}
  \lean{Iut.SourceFGloballyRealifiedMonoAnalyticPrimeStrip}
  \lean{Iut.SourceFGloballyRealifiedMonoAnalyticPrimeStrip.canonical}
  ...
  Definition~2.7 pre-model type includes its standing isotropic-type
  hypothesis, so every object is isotropic in addition to the existence of a
  base--Frobenius pair.  The model proof reuses its established pointwise
  isotropic theorem, and a negative regression proves that a pair alone is
  insufficient when an object is non-isotropic.
  ...
  Given the actual Examples~3.2--3.5 model package, Lean constructs the
  canonical \(D\), \(D^{\sim}\), \(F\), \(F^{\sim}\), and globally realified
  \(F^{\sim}\) strips directly from its constituents.  No further component
  equivalence, prime correspondence, characteristic splitting, or
  currency-exchange map is accepted by these constructors.  The finite
  \(D\)-strip obtained by forgetting \(F\)-structure is compared with the
  canonical \(D_v\) by the model Frobenioid's actual base-category
  equivalence; at infinity the two constituents agree literally.  The
  mono-analytic strips form the Definition~5.2(iii) category using complete
  structured equivalences modulo the Section~0 natural-isomorphism relation,
  with proved identity, composition, and associativity laws and genuine
  finite capsules.
  ...
  source obligations.  Their exact endpoints are separated into
  issues~\#34--\#37 rather than hidden as fields of the canonical
  constructors.
\end{definition}
...
\begin{definition}[Capsules and processions]
  ...
  \lean{Iut.CategoryCapsule.FullPolyMorphism.pairwiseCompositeSet}
  \lean{Iut.CategoryCapsule.InhabitedFullPolyMorphism}
  \lean{Iut.CategoryCapsule.InhabitedFullPolyMorphism.pairwiseCompositeSet_eq_univ}
  \lean{Iut.CategoryCapsule.EmptyIntermediateRegression.pairwiseCompositeSet_ne_univ}
  ...
  formalized directly.  A capsule-full poly-morphism contains the possibly
  empty set of all componentwise isomorphisms over a fixed injection, and its
  pairwise composite is an explicit multiplicity-free range.  An
  \(n\)-procession has a \(j\)-capsule in position \(j\), and its categorical
  morphisms use order-preserving injections together with inhabited
  capsule-full sets.  No member is selected.  Every endpoint member then
  factors through the intermediate capsule, proving that composition is
  exactly the paper's pairwise composite set.  A discrete two-object
  regression records why this inhabitation condition is necessary: two input
  sets may be empty even when the full endpoint set is not.  Capsules and
...
\begin{definition}[Countable temperoids]
  \label{def:countable-temperoids}
  \lean{Iut.SourceTemperoidAction}
  \lean{Iut.SourceConnectedTemperoid}
  \lean{Iut.SourceConnectedTemperoid.connected_iff_nonempty_pretransitive}
  \lean{Iut.SourceConnectedTemperoid.hom_surjective}
  \lean{Iut.SourceTemperoidAction.finiteInclusion}
  \lean{Iut.SourceTemperoidAction.finiteInclusionFullyFaithful}
  \lean{Iut.SourceTemperoidKernelQuotient.connectedAdjunction}
  \lean{Iut.SourceTemperoidKernelQuotient.connectedRestrictionFullyFaithful}
  \leanok
  For a topological group $G$, $B^{\mathrm{temp}}(G)$ is constructed as
  the category of countable discrete sets with a continuous $G$-action.
  Its connected objects are exactly the nonempty transitive actions, and every
  equivariant map between connected objects is an epimorphism.  An open continuous
  surjection \(\Pi\twoheadrightarrow G\) induces fully faithful restriction on
  connected temperoids; its left adjoint divides a \(\Pi\)-action by the
  kernel.  The descended action uses the image of a representative's open
  stabilizer for continuity, so the proof does not assume a finite carrier.
  Finite continuous actions embed fully faithfully in this category.
\end{definition}
...
\begin{definition}[Tempered semi-graph cusp rigidity]
  ...
  \lean{Iut.SourceFiberFunctorComparison.fiberFunctorIso}
  \lean{Iut.SourceAnabelioidHom}
  \lean{Iut.SourceAnabelioidHom.fiberIso}
  \lean{Iut.SourceAnabelioidHom.toPointedHom}
  \lean{Iut.SourceAnabelioidHom.fundamentalGroupHom_innerConjugate_of_fiberIso}
  \lean{Iut.SourceAnabelioidHom.MorphismClass}
  \lean{Iut.SourceOuterHom}
  \lean{Iut.SourceConjugator}
  \lean{Iut.SourceConjugatorCategory}
  \lean{Iut.SourceExactPullbackCategory}
  \lean{Iut.SourceAnabelioidHom.ofContinuousHom}
  \lean{Iut.SourceAnabelioidHom.restrictionConjugatorEquiv}
  \lean{Iut.SourceAnabelioidHom.grothendieckCategoricalEquivalence}
  \lean{Iut.SourceAnabelioidHom.categoricallyEquivalent_restriction_fundamentalGroupHom}
  \lean{Iut.SourceAnabelioidHom.grothendieckCorrespondence}
  \lean{Iut.SourceAnabelioidHom.grothendieckCorrespondence_symm_mk}
  \lean{Iut.SourcePointedAnabelioidHom.BasepointTwoIso}
  \lean{Iut.SourcePointedAnabelioidHom.BasepointTwoIso.fundamentalGroupHom_eq}
  \lean{Iut.SourcePointedAnabelioidHom.PointedMorphismClass}
  \lean{Iut.SourceAnabelioidHom.basepointTwoIso_restriction_fundamentalGroupHom}
  \lean{Iut.SourceAnabelioidHom.pointedGrothendieckCorrespondence}
  \lean{Iut.SourceAnabelioidHom.morphismClassToOuterHom_identity}
  \lean{Iut.SourceAnabelioidHom.morphismClassToOuterHom_comp}
  \lean{Iut.SourcePointedAnabelioidEquivalence.toUnpointedHom_outerHom}
  \lean{Iut.SourceConnectedFiniteEtaleHom.toUnpointedHom_outerHom}
  ...
  Proposition~1.2.1(i) in the continuous-action model.  Conversely, suppose a
  Definition~1.2.2 factorization has connected Galois source.  Lean proves that
  terminal objects of Galois categories are connected and transports this
  property through the slice equivalence.  Every noninitial orbit subobject of
  the slice terminal object is therefore an isomorphism, so the slicing action
  is nonempty and transitive.  Its stabilizer $H$ is open, the action is
  isomorphic to $G/H$, the source is equivalent to $\mathcal B(H)$, and the
  original pullback becomes restriction along $H\leq G$.  For a pointed exact
  equivalence, the quasi-inverse and both triangle $2$-isomorphisms also derive
  a continuous profinite-group equivalence.  Lean now also makes the
  ``appropriate basepoints'' choice of Remark~1.2.2.1: it transports the
  canonical fiber functor of $\mathcal B(H)$ back across the source
  equivalence and certifies $H$ as its fundamental group.  With this basepoint,
  the original pullback is an exact pointed morphism whose derived
  profinite-group map is literally the inclusion $H\hookrightarrow G$.  This
  inclusion is proved to be an open embedding, yielding the connected
  finite-etale morphism asserted by the remark.  For the broader
  Proposition~1.1.4, Lean derives an isomorphism between arbitrary fiber
  functors, defines unpointed morphisms using only exact pullbacks, and proves
  that all comparison choices contribute precisely target inner conjugacy.
  For the standard models $\mathcal B(G)$ and $\mathcal B(H)$, it defines the
  source category of homomorphisms and explicit conjugators and the target
  category of exact pullbacks and natural isomorphisms.  Lemma~1.1.5 supplies
  the bijection on arrows, yielding the categorical equivalence of part~(i);
  the established quotient correspondence is its decategorification and
  preserves identities and composition.  Requiring a natural isomorphism to
  commute with the chosen fiber identifications removes the conjugacy
  ambiguity, yielding part~(ii)'s bijection with actual continuous
  homomorphisms.  The pointed-equivalence and connected finite-etale endpoints
  map to the same outer-hom class.
  ...
  \lean{Iut.SourceEquivalenceBasepoint.etaleFundamentalGroup}
  \lean{Iut.SourceFiniteEtaleFunctorFactorization.connectedPointedHom_fundamentalGroupHom_eq}
  \lean{Iut.SourceFiniteEtaleFunctorFactorization.connectedObjectProfiniteInclusion_isOpenEmbedding}
  \lean{Iut.SourceFiniteEtaleFunctorFactorization.connectedFiniteEtaleHom}
  \lean{Iut.SourcePointedAnabelioidEquivalence.fundamentalGroupContinuousMulEquiv}
  \lean{Iut.sourceSlice_equivalentToGalois_implies_pretransitive}
  \lean{Iut.SourceFiniteEtaleFunctorFactorization.connectedObjectCosetActionIso}
  \lean{Iut.SourceFiniteEtaleFunctorFactorization.connectedSourcePullbackIsoRestriction}
  ...
  deck equivariance for their vertex, edge, and branch maps.  It also constructs
  the canonical proper projection from each normal-open level to the source
  semi-graph.
  
  Lean realizes the combinatorial part of Definition~3.5 and
  Proposition~3.6 through a faithful barycentric incidence graph, with separate
  nodes for vertices, edges, and branches so that loops, parallel incidences,
  and open branches are retained.  Based reduced walks form its universal tree;
  extension performs immediate cancellation one step at a time.  Removing the
  compactification-boundary lifts gives a source semi-graph whose projection is
  a graph covering.

  The construction now consumes the actual pointed connected Galois levels of
  the glued category $\mathcal B(\mathbb G)$.  At a verticial level, the complete
  composite deck group retains the full automorphism group of the Galois object,
  including symmetries invisible on the incidence graph, and couples it to the
  lifted universal-tree permutation.  Its action is faithful on incidence plus
  constituent-fiber data, transitive over the source incidence graph, countable,
  and discrete.  A pointed refinement induces a surjective equivariant incidence
  map and a lawful group homomorphism of complete deck groups commuting with the
  canonical reduced-walk map.  For the isolated-edge case, the finite level is
  already its own combinatorial universal cover; the complete constituent
  automorphism group and its surjective refinement maps are retained.

  The finite-level deck groups now form a lawful group-valued inverse diagram.
  Lean equips its literal inverse limit with the coarsest topology making all
  discrete coordinate projections continuous.  The actual complete deck
  transitions are surjective.  Moreover, bounded lifts of the distinguished
  tree vertex form nonempty finite cofiltered systems: finite-tree compactness
  therefore proves that every literal limit projection is surjective, without
  asserting compactness for arbitrary countable inverse systems.  Thus the
  full deck groups themselves give the required tempered presentation; the
  canonical image presentation remains available and is proved isomorphic and
  homeomorphic to the literal limit.  Finite intersections of coordinate kernels
  form the identity-neighborhood basis, and cofinal changes of level induce
  topological group equivalences, with no stricter canonicity claimed than the
  paper's up-to-inner-automorphism convention.  Thus Proposition~3.6(i) is
  formalized, including the isolated-edge case.
  
  \lean{Iut.SourceSemiGraphSubgroupDiagram.normalOpenLevelProjection}
  \lean{Iut.SourceSemiGraphSubgroupDiagram.normalOpenLevelProjection_isProper}
  \lean{Iut.SourceCombinatorialUniversalCover.UniversalVertex.tree_isTree}
  \lean{Iut.SourceCombinatorialUniversalCover.SourceSemiGraphUniversalCover.projection_isGraphCovering}
  \lean{Iut.SourceCombinatorialUniversalCover.SourceGaloisCombinatorialUniversalCover.deckAction_transitive_compositeFiber}
  \lean{Iut.SourceCombinatorialUniversalCover.SourceGaloisCombinatorialUniversalCover.deckAction_faithful}
  \lean{Iut.SourceCombinatorialUniversalCover.SourceGaloisCombinatorialUniversalCover.refinementIncidenceMap_surjective}
  \lean{Iut.SourceCombinatorialUniversalCover.SourceGaloisCombinatorialUniversalCover.deckTransition}
  \lean{Iut.SourceCombinatorialUniversalCover.SourceIsolatedGaloisCombinatorialUniversalCover.deckTransition_surjective}
  \lean{Iut.SourceCombinatorialUniversalCover.SourceGaloisCombinatorialUniversalCover.literalTemperedPresentation_projection_surjective}
  \lean{Iut.SourceTemperedGroupPresentation.nhds_one_hasBasis_projectionKernels}
  \lean{Iut.SourceTemperedGroupPresentation.cofinalContinuousMulEquiv}
  \lean{Iut.SourceCountableGroupDiagram.imageTransition_surjective}
  \lean{Iut.SourceCountableGroupDiagram.rawLimitContinuousMulEquivImageLimit}
  \lean{Iut.SourceCombinatorialUniversalCover.SourceGaloisCombinatorialUniversalCover.rawDeckLimitContinuousMulEquiv}
  \lean{Iut.SourceCombinatorialUniversalCover.SourceIsolatedGaloisCombinatorialUniversalCover.temperedPresentation}
  \lean{Iut.SourceSemiGraphOfAnabelioids.CovObject}
  \lean{Iut.SourceSemiGraphOfAnabelioids.CovObject.coverProjection}
  \lean{Iut.SourceSemiGraphOfAnabelioids.CovObject.finiteInclusionFullyFaithful}
  \lean{Iut.SourceSemiGraphOfAnabelioids.CovObject.IsTempered}
  \lean{Iut.SourceSemiGraphOfAnabelioids.CovObject.TemperedCover}
  \lean{Iut.SourceSemiGraphOfAnabelioids.CovObject.finiteTemperedInclusionFullyFaithful}
  \lean{Iut.SourceSemiGraphOfAnabelioids.CovObject.temperedCovInclusionFullyFaithful}
  \lean{Iut.SourceSemiGraphOfAnabelioids.CovObject.finiteRecoveryIso}
  \lean{Iut.SourceSemiGraphOfAnabelioids.CovObject.exists_finiteInclusion_preimage}
  \lean{Iut.SourceCombinatorialUniversalCover.SourceGaloisCombinatorialUniversalCover.temperedToProfiniteHom}
  \lean{Iut.SourceCombinatorialUniversalCover.SourceGaloisCombinatorialUniversalCover.continuous_temperedToProfiniteHom}
  
  The literal countable constituent-cover category $B^{\mathrm{cov}}(G)$ is
  now constructed from vertex actions and coherent edge-gluing isomorphisms.
  Its associated countable covering semigraph and projection are explicit.
  Definition~3.5(ii) is represented by the finite constituent kernels that
  split every vertex and edge action; the resulting $B^{\mathrm{temp}}(G)$ is
  a full geometric subcategory, not an alias of an action category.  Finite
  glued covers land in it, and $B(G)\hookrightarrow B^{\mathrm{temp}}(G)$ is
  proved fully faithful.  Conversely, finite constituent actions are lifted
  back through the finite anabelioid equivalences, their edge gluing is
  recovered by full faithfulness, and the resulting object of $B(G)$ maps
  isomorphically to the original geometric cover.  Thus the finite geometric
  part recovers $B(G)$ essentially surjectively as well.  The compatible
  finite Galois symmetries retained in
  every deck coordinate construct a pointed homomorphism from the tempered
  group to the actual automorphism group of the root fiber functor.  Its
  action on each pointed Galois object is given by the corresponding finite
  coordinate, which proves continuity for the inverse-limit topology.  The
  A single globally splitting Galois level now recovers an arbitrary tempered
  geometric cover: evaluation on all maps from its universal cover is
  constituentwise bijective, and the orbits of this generally nontransitive
  deck action retain every countable geometric component.  The remaining
  open portion of Issue~\#54 is the functorial passage between these
  finite-level presentations and the literal inverse-limit action category,
  together with the unit, counit, and triangle laws.
  \lean{Iut.SourceSemiGraphOfAnabelioids.CovObject.AssociatedQuotient.fullTemperedClassification}

  The connected quotient classification is now complete once a geometric
  cover is exhibited as a subcover of one finite-level universal cover.
  Geometric subcovering is represented by an actual morphism from the
  universal cover.  Maps from that source to the connected target carry the
  inverse-precomposition deck action; point-connectedness makes this map set
  countable and transitive.  Evaluation descends through the diagonal
  quotient and is proved constituentwise bijective, hence identifies the
  associated quotient with the independently constructed target.  Moreover,
  associated quotients are point-connected exactly when their auxiliary deck
  actions are transitive.  The remaining geometric step is to derive such a
  finite-level dominating morphism directly from Definition~3.5(ii), before
  the countable-component and inverse-limit assembly.

  The combinatorial path-lifting input for that remaining step is now
  source-independent.  For any cover defined by branch permutations, a
  chosen sheet over the basepoint determines a morphism from the based
  universal semigraph cover.  The composite with the sheet-forgetting
  projection is proved equal to the original universal projection.  Thus the
  forthcoming finite-split domination argument may use the paper's ordinary
  graph-cover lift rather than accepting a lift as additional data.
...
\begin{definition}[Mono-theta environments and inverse-limit group]
  ...
  \lean{Iut.SourceProfiniteSubquotient}
  \lean{Iut.SourceProfiniteSubquotient.toTopologicalSubquotient}
  \lean{Iut.SourceProfiniteSubquotient.nonclosed_denominator_not_canonical}
  ...
  \lean{Iut.EtaleTheta.ProfiniteLowerCentralTopology.thetaQuotient_isSES}
  \lean{Iut.EtaleTheta.ProfiniteLowerCentralTopology.ellipticAbelianization_isSES}
  \lean{Iut.EtaleTheta.ProfiniteLowerCentralTopology.modLThetaQuotient_isSES}
  \lean{Iut.EtaleTheta.ProfiniteLowerCentralTopology.modLEllipticAbelianization_isSES}
  \lean{Iut.EtaleTheta.ProfiniteLowerCentralTopology.thetaCenter_isSES}
  \lean{Iut.EtaleTheta.ProfiniteLowerCentralTopology.lDeltaThetaSubquotientContinuousMulEquiv}
  ...
  \lean{Iut.GlobalLTorsionCoverInput.arithmeticThetaQuotientExact_isSES}
  \lean{Iut.GlobalLTorsionCoverInput.arithmeticEllipticQuotientExact_isSES}
  ...
  by a normal subgroup.  The \(\ell\Delta^\Theta\) coefficient has the stronger
  certified profinite boundary: its numerator and denominator are closed, its
  quotient is homeomorphic to an actual profinite group, and a nonclosed
  denominator cannot instantiate this field.  Pullback is the derived quotient
  ...
  homomorphism on that center, and is consequently central, normal, and closed
  as a compact image.  Pullback
  ...
  resulting geometric subquotient with \(\ell\Delta^\Theta\) by a
  homeomorphism.  Its numerator and denominator are transported through the
  injective geometric-to-arithmetic profinite map, preserving closedness and
  giving an actual arithmetic-image subquotient homeomorphic to the geometric
  coefficient group.  For a subgraph inclusion,
...
\begin{definition}[Continuous theta-value germs and Gaussian monoids]
  ...
  \lean{Iut.SourceKummerFaithfulness.FiniteExtension}
  \lean{Iut.SourceKummerFaithfulness.SemiAbelianRationalPointFamily}
  \lean{Iut.SourceKummerFaithfulness.TorusRationalPointFamily}
  \lean{Iut.SourceKummerFaithfulness.KummerFaithful}
  \lean{Iut.SourceKummerFaithfulness.TorallyKummerFaithful}
  \lean{Iut.SourceKummerFaithfulness.AdditiveKummerMap.injective_of_trivial_infinitelyDivisible}
  \lean{Iut.SourceKummerFaithfulness.MultiplicativeKummerMap.injective_of_roots_eq_one}
  ...
  \lean{Iut.SourceMLFKummerFaithfulness.fieldUnit_eq_one_of_roots}
  \lean{Iut.SourceMLFKummerFaithfulness.finiteExtensionGMUnit_eq_one_of_roots}
  \lean{Iut.SourceMLFKummerFaithfulness.finiteExtensionGMKummerMap_injective}
  ...
  quotient.  This proves precisely the integral-unit input used by the
  $M_{\rm TM}^{\times}$ map; it is not identified with the full toral
  condition.  Separately, Lean proves the full $\mathbb G_m(L)=L^\times$
  case for every finite local-field extension $L/K$: divisibility by every
  positive integer first forces the discrete valuation to vanish, and the
  valuation-ring-unit theorem then forces the element itself to be one.
  Kummer-map injectivity is derived from this root rigidity and the standard
  kernel characterization, rather than stored.  Actual arbitrary-torus and
  semi-abelian rational-point families remain explicit obligations.  For a
  compact IUT acting group, Lean also proves that the image of
...
\begin{definition}[Source-shaped Theorem 3.11 interfaces]
  ...
  \lean{Iut.SourceTheorem311Ind3DirectionalCompatibility.map}
  \lean{Iut.sourceTheorem311StrictBoolInd3_not_quotient_eq}
  \leanok
  ...
  Horizontal evaluation descends only through the exact \(\Ind{1}/\Ind{2}\)
  quotient.  The \(\Ind{3}\) interface preserves its one-way orientation into
  inclusions, relational surjections, or log-volume inequalities; a strict
  two-point regression shows that a forward \(\Ind{3}\) step is not quotient
  equality.
...
\begin{definition}[Indeterminacy generator interface]
  ...
  \lean{Iut.Stage1.IUTStage1Theorem311IndeterminacySourceData,Iut.Stage1.IUTStage1Theorem311IndeterminacySourceData.DirectionalReachability,Iut.Stage1.IUTStage1Theorem311IndeterminacySourceData.DirectionalReachability.ind3}
  \leanok
  The Stage 1 interface names the three source-facing indeterminacy
  relations separately: procession automorphism, local tensor symmetry, and
  upper-semi compatibility.  Only the first two generate the equality
  quotient.  The third generates one-way directional reachability, never a
  symmetric identification.  This is a Lean-checked naming interface, not yet
